...
export default function EscalationDashboard() {
  ...
  const handleFilesSelected = (filesList) => {
    ...
    for (let i = 0; i < take; i++) {
      ...
      if (f.size > MAX_BYTES) {
        setImgError(`Each image must be ≤ ${MAX_MB} MB. "${f.name}" is too large.`);
        continue;
      }
      accepted.push(f);
    ...
    if (files.length > take) setImgError(`Maximum ${MAX_IMAGES} images allowed.`);
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-purple-700 mb-4">Escalation Dashboard</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={closeModal}>
          <div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="max-h-[80vh] overflow-y-auto px-5 py-4 space-y-6">
              ...
              {/* Shipment details */}
              <section>
                <h3 className="text-lg font-semibold text-purple-700 mb-3">Shipment Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                  <Info label="Consignor Name" value={shipment?.consignorname || "-"} />
                  <Info label="Consignor Phone No" value={shipment?.consignorphonenumber || "-"} />
                  <Info label="Consignee Name" value={shipment?.consigneename || "-"} />
                  <Info label="Consignee Phone No" value={shipment?.consigneephonenumber || "-"} />
